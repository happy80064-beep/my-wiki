...
1. **关系类型误判**：当原文涉及模型、工具、框架、技术组件时，AI 容易把 Gemini、SenseVoice-Small、OpenCLI、OpenClaw、Fish Audio S2、Live2D/Pixi 等技术组件误标为“负责人”。后续需要扩展关系类型和提示词，把技术组件表达为“使用 / 依赖 / 组件 / 方案候选”，只有 person 实体才能使用“负责人 / 参与者”等人类职责关系。
2. **任务项漏提取**：当原文使用“短期优先级”“推荐后续路线”“下一阶段核心”等路线规划表达时，AI 可能只当作摘要或主题素材，没有生成 tasks。后续需要强化任务识别规则：编号行动项、带有“稳住 / 完善 / 强化 / 保留 / 评估 / 做稳 / 让…成为…”等动词的计划项，应生成可编辑任务草稿；无法确定 owner 时标记为 uncertain 并交给用户确认。
