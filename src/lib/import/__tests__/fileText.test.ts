...
describe('import file kind detection', () => {
  it('detects text, word, spreadsheet, html, pdf, and image files', () => {
    ...
    expect(getImportFileKind('访谈.mp3')).toBeUndefined();
    expect(getImportFileKind('会议.mp4')).toBeUndefined();
  });
  ...
  it('uses mime type as a fallback', () => {
    ...
    expect(getImportFileKind('untitled', 'audio/mpeg')).toBeUndefined();
    expect(getImportFileKind('untitled', 'video/mp4')).toBeUndefined();
