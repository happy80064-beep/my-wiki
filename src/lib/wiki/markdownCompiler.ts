...
export function buildWikiMarkdownBatchCompilePrompt(input: WikiMarkdownBatchCompileInput) {
  ...
  return [
    ...
    '- 重要数字必须保留完整单位和证据语境。',
    '- 使用 Markdown 表格呈现关键指标；没有指标时写“当前来源未提供明确指标”。',
    '- 即使目标页面只是项目下的子板块、业务线、概念或薄证据主题，也必须生成完整页面；证据不足的章节要明确写“当前来源未确认”，不要省略 FILE block，也不要只写一两句话。',
    '- 每篇正文必须至少包含这些二级标题：摘要、项目/主题概述、关键事实、结构与板块、关键指标、来源与证据、未确认与待补充、相关页面。',
    '- 每篇页面长度应足够支撑查询检索：至少写出 3 个以上二级标题；如果只有一句来源证据，也要围绕该证据说明其在项目/主题中的位置、已确认事实、未确认信息和来源。',
    '',
    '## 输出格式示例',
    '```',
