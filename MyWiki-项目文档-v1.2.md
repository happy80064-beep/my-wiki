...
实现要求：
- 先从问题中抽取属性关键词和同义词，如"唤醒词 / 叫醒词 / KWS"、"终止词 / 停止词 / 打断词"、"API key"、"模型"、"路径"、"负责人"。
...
- 找到答案后，应产生一个"待编译回 Wiki"的建议，例如把 `桌面数字生命体.properties.wakeWord = 小林`，或创建"唤醒词"主题并建立关系。MVP 可以先只提示，后续再做一键确认写回。
...
- AI 查询（渐进式 Wiki 阅读式查询：目录 → 实体文档 → 关系子图 → 来源证据 → LLM 表达）
