...
#### 4.7.4 Froggy Capture 拟物化桌面入口

目标：把“打开网页 -> 导航到捕获页 -> 拖拽/选择文件 -> 等待 AI 处理 -> 查看状态”的多步操作，压缩成一个常驻小入口。用户把文件、截图或剪贴板内容拖进“捕获蛙”嘴里，系统先完成 Raw Inbox 收件，再按用户设置异步编译。
...
/src
  /pages                # 当前 Vite + React 页面
    CapturePage.tsx     # 捕获页
    FrogWidgetPage.tsx  # 拟物化捕获入口（Web Widget MVP）
    WikiPage.tsx        # 知识库浏览
    EntityPage.tsx      # 实体详情
    GraphPage.tsx       # 关系图谱
    QueryPage.tsx       # AI 查询
...
目标：停止在查询时从 OCR/PDF 原文里临时猜数值，把“项目、业态、版块下的面积、收入、投资、人数、时间”等事实属性前移到摄入/编译阶段。

核心设计：

1. **实体新增 `indicators` 指标层**：项目、主题等实体可保存结构化指标，每条指标包含 `name / value / unit / businessLine / categoryName / source / confidence / note`。例如“住宅板块建筑面积”“医疗板块用地面积”“稳定运营期年均总收入”。
2. **`value = null` 是有效事实**：如果材料明确没有给出某个指标，系统应写入 `value: null` 和 `note`，代表“已查过，原文未提供”。这比查询时反复从噪声里猜数字更安全。
3. **摄入时做属性预编译**：AI 编译长文档时，应识别“业态/板块/维度”结构，并对每个维度主动尝试提取常见指标：用地面积、建筑面积、投资额、收入、人数、时间节点等。
4. **证据强绑定**：每个指标必须带来源 entry、原文摘录、页码/章节（如能解析）和置信度。没有“维度名 + 指标名 + 数值 + 单位”的完整上下文，不得标为高置信。
5. **查询优先读指标层**：fact lookup 类问题先解析目标实体、维度和指标名，再读取 `entity.indicators`。有值则直接回答；`value = null` 则明确说明资料未提供；只有旧数据缺指标字段时，才允许进入原文兜底扫描。
6. **兜底结果必须回流**：从原始材料兜底扫出的疑似指标不得直接沉默写入，而是进入待编译回 Wiki 队列，用户确认后再成为正式指标。
7. **避免查询层无限补丁**：不要继续用越来越复杂的正则在查询时猜“住宅面积/医疗用地/投资额/人数”等事实。正则只用于兜底发现线索，不作为新数据的主路径。
8. **未知维度不得套用**：用户问题里的维度、业态、板块或对象必须先在 `categories / indicators / aliases / sourceEntries` 中命中；如果用户问“智算中心面积”但 Wiki 和来源里没有“智算中心”，系统必须明确说未找到该维度，不得把住宅、医疗、文旅等其他板块的面积套用为答案，也不得生成待编译写回建议。
9. **同句多数字按维度邻近选择**：原文兜底只在旧数据缺指标时使用；当同一句里出现多个面积/金额数字时，必须优先选择距离用户询问维度最近的数值。例如“药材科普园 132 亩建设用地和文旅开发空地 685 亩”用于回答文旅面积时，应优先返回 `685 亩`，而不是句首第一个 `132 亩`。

触发条件：

- 文档正文超过 2000 字，或来源类型为 PDF / Word / 图片解析结果，默认进入指标预编译检查。
- 文档标题、目录或正文出现“项目、园区、业态、板块、产品线、经营测算、收入、投资、面积、人数、床位、时间节点”等信号时，必须尝试提取 indicators。
- 用户在实体页或 Raw Inbox 主动点击“回填属性/编译新材料”时，即使文档较短，也应重新检查是否存在可结构化指标。
- 普通短文本捕获仍可走轻量提取，但如果出现明确“维度名 + 指标名 + 数值 + 单位”，也应写入 indicators。

长文档策略：

- 15000 字以内：先按章节/标题切分，再做一次“结构理解 → 指标提取”的两步 Prompt。
- 超过 15000 字：按 Markdown heading、页码或自然段分块，每块只提取本块出现的候选 indicators；最后再做一次合并去重，统一 categoryName、businessLine、source 和 confidence。
- 分块时不得切开表格、代码块、图表说明和同一页的关键上下文；每个候选指标必须保留 100-200 字证据窗口。
- 如果同一指标在多块中冲突，不自动覆盖；进入审核队列，显示多个候选值和来源。

指标提取 Prompt 模板（MVP 可直接落地）：

```text
你是 MyWiki 的摄入编译器。请只根据给定材料提取项目/主题的结构化指标。

任务：
1. 识别材料中是否存在业态、板块、产品线、阶段、区域等层级结构。
2. 对每个维度尝试提取常见指标：用地面积、建筑面积、投资额、收入、成本、人数、床位、时间节点、状态、负责人。
3. 每个指标必须输出：
   - name：指标名，如“住宅板块建筑面积”
   - value：数字；如果材料明确没有提供，填 null
   - rawValue：原文里的完整数值表达，如“248 亩”“4.22 亿元”
   - unit：单位，如“亩”“万平方米”“亿元”
   - businessLine：业务线短名，如“住宅”“医疗”
   - categoryName：必须对应 categories.name，如“住宅业态”“医疗业态”
   - source.entryId / section / page / excerpt
   - confidence：high / medium / low
   - note：不确定原因或未找到说明

硬约束：
- 不要从目录、页码、章节号、图表编号、页眉页脚、导航文字中抽数字。
- 没有完整“维度名 + 指标名 + 数值 + 单位”上下文时，不得标为 high。
- 不要把一个业态的数字归给另一个业态；例如医疗板块 248 亩不能回答住宅建筑面积。
- 如果只看到“21 万”但缺少单位或字段对应关系，value 仍可为空，rawValue 记录“21 万”，confidence 标为 low，并进入审核。
- value = null 是有效事实，代表已经检查过但材料未提供，不是提取失败。
- 输出只允许 JSON，不要添加解释文字。
```

审核界面规范：

- 指标提取完成后，用户应看到“待确认指标”列表，可对每条执行确认、修改、删除。
- 每条指标必须展示：实体、业态/板块、指标名、建议值、单位、置信度、证据摘录和来源入口。
- 用户确认后写入 `entity.indicators`；修改后写入用户修正值，并保留原始 `rawValue`；删除/忽略后不得在同一来源和同一字段上反复弹出。
- 低置信指标默认不进入正式回答，只作为查询兜底线索和待编译项。

最小数据形态：

```ts
type EntityIndicator = {
  id: ID;
  name: string;
  value: number | null;
  rawValue?: string;
...
- 问“福瑞三期住宅项目建筑面积是多少”时，如指标层标注未提供，应直接回答未找到明确值，不得把医疗板块 `248 亩`、目录编号或 OCR 碎片误当答案。
- 问“福瑞三期医疗板块用地面积是多少”时，如指标层有 `248 亩`，应直接读取指标层回答，并展示对应来源。
- 同一指标确认写回后，再次查询不得重复生成相同待编译建议。
- 问“福瑞三期医疗和住宅业态的面积分别是多少？”时，应按业态分组返回已编译指标；医疗可返回 `248 亩`，住宅若 `value = null` 则明确说未提供，不得互相串用。
- 问“福瑞三期主要指标有哪些？”时，应按 `categoryName` / `businessLine` 分组列出 indicators，并标注哪些指标有值、哪些指标未提供明确数值。
- 问“福瑞三期医疗方面的用地面积是多少？”时，应能把“医疗方面”模糊匹配到 `医疗业态`，再读取该 category 下的指标。
- 问“福瑞三期中智算中心的面积是多少？”且材料中没有“智算中心”时，应明确说未找到该维度，不得返回住宅、文旅、医疗等其他业态面积，也不得生成待编译写回建议。
- 问“福瑞三期文旅业态的面积是多少？”且同一来源句同时出现多个面积数字时，应优先采用与“文旅”最近的面积值，并在来源中保留可核对摘录。

验证顺序：先用“福瑞三期”的真实材料手工构造 `categories` 和 `indicators`，不调用 LLM，跑通上述查询测试；只有当手工数据能稳定查对，再接入 LLM 自动提取。这样可以把数据建模/查询路径问题和 LLM 摄入误差分开排查。

### 14.3.3 层级结构（categories）建模与指标联动

目标：把“父实体 -> 业态/板块/维度 -> 项目/服务/机构/指标”的结构显式存入 Wiki，让查询不再只靠扁平关系和原文扫描猜测。

最小数据形态：

```ts
type EntityCategory = {
  name: string;                 // 如“医疗业态”“住宅业态”
  aliases?: string[];           // 如“医疗”“医疗方面”“医疗板块”
  items: {
    title: string;              // 子项目/服务/机构名
    entityId?: ID;              // 已升级为独立实体时填写
    kind?: string;              // project / service / institution / method 等
    summary?: string;
    evidence?: string;
  }[];
  evidence?: string;
...
1. **categories 与 indicators 并列存在**：`categories` 表达层级结构，`indicators` 表达该层级下的事实指标。两者不互相替代。
2. **字段必须对齐**：`indicator.categoryName` 必须优先对应某个 `categories[].name`；`indicator.businessLine` 是短名，可由 `categoryName` 去掉“业态/板块/版块/业务线”等后缀得到，例如 `住宅业态 -> 住宅`。
3. **别名参与查询，不参与主键**：用户问“医疗方面”“医疗板块”“医疗业务”时，可通过 `categories.aliases` 命中 `医疗业态`，但写回时仍统一保存为 `categoryName: "医疗业态"`。
4. **不一致时进入审核**：如果 LLM 输出的 `indicator.categoryName` 找不到对应 category，系统不得沉默写入；应进入待确认队列，提示用户选择现有 category 或创建新 category。
5. **按需升级实体**：`医疗业态` 这类维度 MVP 可作为父实体内部 category；只有当用户频繁深入查询或需要独立关系网络时，再升级为独立 entity，并把 category items 迁移/链接过去。
6. **查询优先下钻**：列表类问题先走 `drillDown(parent, category)`；事实类问题先走 `entity.indicators` 并按 `categoryName/businessLine` 过滤；旧数据缺失时才回扫相关 sourceEntries。
7. **未知 category 安全失败**：如果用户提出的 category / 维度无法匹配任何 `categories.name / aliases / indicators.categoryName / sourceEntries`，查询应停止在“未知维度”状态，不进入通用指标匹配。

MVP 验收：

- `福瑞三期医疗业态中有哪些项目？` 应返回 `医疗业态.items`，而不是从目录页噪声中抽取“实现业务协同”等章节标题。
- `福瑞三期医疗和住宅业态的面积分别是多少？` 应同时读取两个 category 下的 indicators。
- `福瑞三期医疗方面的用地面积是多少？` 应通过 alias 命中 `医疗业态`。
- 当 `indicator.categoryName = "医疗板块"` 但 categories 中只有 `医疗业态` 且 alias 包含“医疗板块”时，写入前应规范化为 `医疗业态`。
...
- v1.2 补充：新增指标层摄入路线，实体可选 `indicators` 保存数值/事实指标；查询 fact lookup 优先读指标层，`value = null` 代表资料已确认未提供，原文抽取只作为旧数据低置信兜底，避免住宅面积、医疗用地、收入、人数字段在查询时被 OCR 噪声误配
